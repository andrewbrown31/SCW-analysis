...
!-----------------------------------------------------------------------
    ...
    subroutine getcape( nk , p_in , t_in , td_in, pinc, source, ml_depth, adiabat, cape , cin )
    implicit none

    integer, intent(in) :: nk,source,adiabat
    real, intent(in) :: pinc,ml_depth
    ...
    do while( doit .and. (k.lt.nk) )
      ...
      do n=1,nloop
        ...
        do while( not_converged )
          ...
          th2=th1*exp(  lhv*(ql2-ql1)/(cpm*tbar)+lhs*(qi2-qi1)/(cpm*tbar)+(rm/cpm-rd/cp)*alog(p2/p1) )
