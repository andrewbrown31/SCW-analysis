...
def plot_boxplot(v_list, models, data_list, cmip6, barpa, nrm_da, spatial_diff, spatial_diff_sig, mean_envs, ens_df, min_envs=0, plot_map=True):
	...
	ranges = [-7.5,6]
	ranges2 = [-100,100]
	lab = {"logit_aws":"BDSD", "dcp":"DCP", "t_totals":"T-Totals", "eff_sherb":"SHERBE"}
	#For each NRM region...
	for i in np.arange(len(data_list[0])):
		...
		for month in return_months():
			#For each variable (with a dummy variable for plot spacing)...
			for v in np.arange(len(v_list)+1):
				if v == len(v_list):
					temp = np.ones(12)*-999
				else:
					temp = data_list[v][i].loc[month].values
				#Get summary stats for each diagnostic/month, based on ensemble of GCMs
		...
		#Append annual stats (relative change) for all diagnostics and GCMs combined
		...
		#Append annual stats (relative change) for just the statistical diagnostic
		boxes.append({"med":np.percentile(ens_df[i].loc["logit_aws"], 50, interpolation="nearest"),\
			"q1":np.percentile(ens_df[i].loc["logit_aws"], 16.67, interpolation="nearest"),\
			"q3":np.percentile(ens_df[i].loc["logit_aws"], 83.33, interpolation="nearest"),\
			"whislo":ens_df[i].loc["logit_aws"].values.min(),\
			"whishi":ens_df[i].loc["logit_aws"].values.max()})
		#For each diagnostic/NRM region, plot a line joining the median
		for v in np.arange(len(v_list)+1):
			if v < len(v_list):
				ax.plot(np.arange(0,12)*(len(v_list)+1) + (v+1), np.percentile(data_list[v][i], 50, interpolation="nearest", axis=1),lw=2, ls=":")
		#Plot monthly summary stats as a boxplot, and color each box
		box=ax.bxp(boxes[0:len(boxes)-2], showfliers=False)
		bcnt = 0
		for b in box["boxes"]:
		...
		#Plot annual summary stats (for ensemble of diagnostics, and statistical diagnostic) on a separate panel
		ax2=plt.subplot2grid((5,5), (i, 4), colspan=1, rowspan=1)
		box2=ax2.bxp([boxes[-2]], showfliers=False, widths=0.4, positions=[2])
		box2["boxes"][0].set_color("k")
		box2["medians"][0].set_color("k")
		box3=ax2.bxp([boxes[-1]], showfliers=False, widths=0.4, positions=[1])
		box3["boxes"][0].set_color("tab:blue")
		box3["medians"][0].set_color("tab:blue")
		...
		#Code to plot additional CMIP6 data
		#cmip6plot = []
		...
		if i == 3:
			...
			for v in np.arange(len(v_list)):
				dummy_lines.append(mlines.Line2D([], [], color=cols[v], label=lab[v_list[v]]))
			ax.legend(handles=dummy_lines, ncol=4,loc="upper center", fontsize=12)
...
def plot_spatial_diffs(v_list, models, nrm_da, spatial_diffs, spatial_diff_sigs, mean_envs, min_envs=1):
	...
	for v in np.arange(len(v_list)):
		...
		for spatial_diff, spatial_diff_sig in zip(spatial_diffs[v], spatial_diff_sigs[v]):
			...
			c=xr.plot.contourf(xr.where((~(nrm_da["aus"].isnull())), spatial_diff, np.nan), cmap=plt.get_cmap("RdBu_r"), vmin=-9, vmax=9, levels=10, extend="both", ax=ax, add_colorbar=False, add_labels=False)
...
def get_diff(hist, scenario, models, shapes, nrm, rel=True, period="all", var=None):
...
if __name__ == "__main__":
	...
	lat1 = -45; lat2 = -10
	experiment = "rcp85"
	v_list = ["logit_aws","t_totals","eff_sherb","dcp"]
	...
	change0_rel_djf = pd.DataFrame()
	change1_rel_djf = pd.DataFrame()
	change2_rel_djf = pd.DataFrame()
	change3_rel_djf = pd.DataFrame()
	...
	for v in v_list:
		...
		change0_rel_djf = pd.concat([change0_rel_djf, get_diff(hist[cnt], scenario[cnt], models, shapes, [0], rel=True, period="djf", var=v_list[cnt])[0]], axis=0)
		change1_rel_djf = pd.concat([change1_rel_djf, get_diff(hist[cnt], scenario[cnt], models, shapes, [1], rel=True, period="djf", var=v_list[cnt])[0]], axis=0)
		change2_rel_djf = pd.concat([change2_rel_djf, get_diff(hist[cnt], scenario[cnt], models, shapes, [2], rel=True, period="djf", var=v_list[cnt])[0]], axis=0)
		change3_rel_djf = pd.concat([change3_rel_djf, get_diff(hist[cnt], scenario[cnt], models, shapes, [3], rel=True, period="djf", var=v_list[cnt])[0]], axis=0)
		change0_rel = pd.concat([change0_rel, get_diff(hist[cnt], scenario[cnt], models, shapes, [0], rel=True, period="annual", var=v_list[cnt])[0]], axis=0)
		change1_rel = pd.concat([change1_rel, get_diff(hist[cnt], scenario[cnt], models, shapes, [1], rel=True, period="annual", var=v_list[cnt])[0]], axis=0)
		change2_rel = pd.concat([change2_rel, get_diff(hist[cnt], scenario[cnt], models, shapes, [2], rel=True, period="annual", var=v_list[cnt])[0]], axis=0)
		change3_rel = pd.concat([change3_rel, get_diff(hist[cnt], scenario[cnt], models, shapes, [3], rel=True, period="annual", var=v_list[cnt])[0]], axis=0)
		change0_abs = pd.concat([change0_abs, get_diff(hist[cnt], scenario[cnt], models, shapes, [0], rel=False, period="annual", var=v_list[cnt])[0]], axis=0)
		change1_abs = pd.concat([change1_abs, get_diff(hist[cnt], scenario[cnt], models, shapes, [1], rel=False, period="annual", var=v_list[cnt])[0]], axis=0)
		change2_abs = pd.concat([change2_abs, get_diff(hist[cnt], scenario[cnt], models, shapes, [2], rel=False, period="annual", var=v_list[cnt])[0]], axis=0)
		change3_abs = pd.concat([change3_abs, get_diff(hist[cnt], scenario[cnt], models, shapes, [3], rel=False, period="annual", var=v_list[cnt])[0]], axis=0)
		#if v == "logit_aws":
		#	stat0_rel,_,_ = get_diff(hist[cnt], scenario[cnt], models, shapes, [0], rel=True, annual=True, var=v_list[cnt])
		#	stat1_rel,_,_ = get_diff(hist[cnt], scenario[cnt], models, shapes, [1], rel=True, annual=True, var=v_list[cnt])
		#	stat2_rel,_,_ = get_diff(hist[cnt], scenario[cnt], models, shapes, [2], rel=True, annual=True, var=v_list[cnt])
		#	stat3_rel,_,_ = get_diff(hist[cnt], scenario[cnt], models, shapes, [3], rel=True, annual=True, var=v_list[cnt])
	...
	change_rel = pd.DataFrame({\
		"NA":[np.quantile(np.array(change0_rel_djf), 0.1), np.quantile(np.array(change0_rel_djf), 0.5),np.quantile(np.array(change0_rel_djf), 0.9)],\
		"RA":[np.quantile(np.array(change1_rel_djf), 0.1), np.quantile(np.array(change1_rel_djf), 0.5),np.quantile(np.array(change1_rel_djf), 0.9)],\
		"EA":[np.quantile(np.array(change2_rel_djf), 0.1), np.quantile(np.array(change2_rel_djf), 0.5),np.quantile(np.array(change2_rel_djf), 0.9)],\
		"SA":[np.quantile(np.array(change3_rel_djf), 0.1), np.quantile(np.array(change3_rel_djf), 0.5),np.quantile(np.array(change3_rel_djf), 0.9)]})
	...
	change0_rel.to_csv("/g/data/eg3/ab4502/figs/CMIP/rel_nrm0_changes.csv")
	change1_rel.to_csv("/g/data/eg3/ab4502/figs/CMIP/rel_nrm1_changes.csv")
	change2_rel.to_csv("/g/data/eg3/ab4502/figs/CMIP/rel_nrm2_changes.csv")
	change3_rel.to_csv("/g/data/eg3/ab4502/figs/CMIP/rel_nrm3_changes.csv")
