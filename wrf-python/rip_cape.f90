...
SUBROUTINE DCAPECALC3D(prs,tmk,qvp,ght,ter,sfp,cape,cin,&
            lfc, lcl, el, prsf, prs_new, tmk_new, qvp_new, ght_new,&
    ...
    !f2py threadsafe
    !f2py intent(in,out) :: cape, cin, lfc, lcl, el
    ...
    REAL(KIND=8), DIMENSION(mix,mjy,mkzh), INTENT(OUT) :: lfc
    REAL(KIND=8), DIMENSION(mix,mjy,mkzh), INTENT(OUT) :: lcl
    REAL(KIND=8), DIMENSION(mix,mjy,mkzh), INTENT(OUT) :: el
    ...
    DO j = 1,mjy
        DO i = 1,mix
            ...
            DO kpar = 2, mkzh
                ...
                !CHANGE AB
                !If no EL is found, then why would we want to set CAPE to 0.1 or
                !missing? It would be more appropriate to calculate CAPE
                !approximating the EL as the top model level.
                !NOTE that cycle is also commented out
                ...
                cape(i,j,kpar) = MAX(benaccum(kel)-benamin, 0.0D0)
                cin(i,j,kpar) = MAX(-benamin, 0.0D0)
                lfc(i,j,kpar) = zrel(klfc) + ght_new(kpar,i,j) - ter(i,j)
                lcl(i,j,kpar) = zrel(klcl) + ght_new(kpar,i,j) - ter(i,j)
                !lcl(i,j,kpar) = zlcl - ter(i,j)
                IF (.NOT. elfound) THEN
                    el(i,j,kpar) = zrel(klcl) + ght_new(kpar,i,j) - ter(i,j)
                ELSE
                    el(i,j,kpar) = zrel(kel) + ght_new(kpar,i,j) - ter(i,j)
                ENDIF
                ...
                IF (cape(i,j,kpar) .EQ. 0.D0) cin(i,j,kpar) = 0.D0
...
! NCLFORTEND
    ...
    DO j = 1,mjy
        DO i = 1,mix
            ...
            IF (.NOT. elfound) THEN
                cin(i,j,mkzh-2) = cmsg
                cin(i,j,mkzh-3) = zrel(klfc) + ght_new(kpar1,i,j) - ter(i,j)
            ELSE
                cin(i,j,mkzh-2) = zrel(klfc) + ght_new(kpar1,i,j) - ter(i,j)
                cin(i,j,mkzh-3) = zrel(kel) + ght_new(kpar1,i,j) - ter(i,j)
            ENDIF
        END DO
    END DO
    !$OMP END PARALLEL DO
