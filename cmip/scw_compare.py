...
def load_resampled_logit(model, ensemble, p2=False):
	...
	if p2:
		try:
			...
			model_6hr = xr.open_mfdataset("/g/data/eg3/ab4502/ExtremeWind/aus/regrid_1.5/"+\
			    model+"_"+ensemble+"_logit_aws_rcp85_1979_2005.nc")
	else:
		model_6hr = xr.open_mfdataset("/g/data/eg3/ab4502/ExtremeWind/aus/regrid_1.5/"+\
...
if __name__ == "__main__":
	...
	if not load:
		...
		for nrm in [ [0], [1], [2], [3] ]:
			...
			data.append([df, df2, era5_df, era5_df2, era5_df3])
	...
	for i in np.arange(4):
		...
		if seaborn is True:
			...
			for n, g in group:
				g.plot(x="time_ind",y="logit",ax=ax,color="grey", alpha=0.4, legend=False)
			for n, g in group2:
				g.plot(x="time_ind",y="logit",ax=ax,color="grey", alpha=0.4, legend=False)
			era5_df3.plot(x="time_ind",y="logit",ax=ax,color="tab:red", legend=False, marker="o")
			df.groupby("time_ind").median()["logit"].plot(color="k", legend=False, ax=ax, marker="o")
			df2.groupby("time_ind").median()["logit"].plot(color="k", legend=False, ax=ax, marker="o")
		plt.xlabel("")
