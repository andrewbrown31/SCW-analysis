...
def read_barra_r_fc(domain,times,wg_only):
	...
	+year+month+day+"T"+hour+"*.nc")[0])
		...
		date_times = np.append(date_times,\
			nc.num2date(np.round(wg_file["time"][:]),wg_file["time"].units))
		print(date_list[t])

		temp_wg = wg_file["max_wndgust10m"][:,lat_ind,lon_ind]
		wg = np.append(wg,temp_wg,axis=0)
		wg_file.close()
	...
	if wg_only:
		return [wg,lon,lat,date_times]
	else:
		return [wg,ta,dp,hur,hgt,terrain,pres,ps,ua,va,uas,vas,lon,lat,date_times]
