...
def load_resampled_era5(p, thresh):
	if p == "dcp":
		var = "dcp"
	elif p == "scp":
		var = "scp_fixed"
...
if __name__ == "__main__":
	...
	cnt=1
	alph = ["","a","b","c"]
	thresh = {"logit_aws":0.72, "dcp":0.04, "scp":0.04, "cs6":26222, "mu_cape":230}

	seasons = {"DJF":[12,1,2], "MAM":[3,4,5], "JJA":[6,7,8], "SON":[9,10,11]}
	y1=1979; y2=1992; y3=1992; y4=2005

	if compute:
		for p in ["dcp","cs6","mu_cape"]:

			ProgressBar().register()
			era5 = load_resampled_era5(p, thresh).persist()#.resample({"time":"1D"}).max()
			access0 = load_logit("ACCESS1-0","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			access3 = load_logit("ACCESS1-3","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			bnu = load_logit("BNU-ESM","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			cnrm = load_logit("CNRM-CM5","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			gfdl_cm3 = load_logit("GFDL-CM3","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			gfdl2g = load_logit("GFDL-ESM2G","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			gfdl2m = load_logit("GFDL-ESM2M","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			ipsll = load_logit("IPSL-CM5A-LR","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			ipslm = load_logit("IPSL-CM5A-MR","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			miroc = load_logit("MIROC5","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			mri = load_logit("MRI-CGCM3","r1i1p1",p).persist().resample({"time":"1D"}).max("time")
			bcc = load_logit("bcc-csm1-1","r1i1p1",p).persist().resample({"time":"1D"}).max("time")

			x = [access0, access3, bnu, cnrm, gfdl_cm3, gfdl2g, gfdl2m, ipsll, ipslm, miroc, mri, bcc]
			n = ["ACCESS1-0","ACCESS1-3","BNU-ESM","CNRM","GFDL-CM3","GFDL-ESM2G","GFDL-ESM2M"\
				,"IPSL-CM5A-LR","IPSL-CM5A-MR","MIROC5","MRI-CGCM3","bcc-csm1-1"]
			...
			for s in ["DJF","MAM","JJA","SON"]:
				...
				cmip_sig = np.where( ( ( (change > 0) & (spatial_diff_pos >= 10) ) |\
					 ( (change <= 0) & (spatial_diff_neg >= 10) ) ) & (mean1 >= 1), 1, 0) 
				...
				xr.Dataset(data_vars={"cmip_trend":\
					...
					coords={"lat":x[0].lat.values, "lon":x[0].lon.values}).to_netcdf(\
					"/g/data/eg3/ab4502/ExtremeWind/trends/spatial_hist_trend_"+s+"_"+p+".nc")
	...
	for season in ["DJF","MAM","JJA","SON"]:
			...
			f1 = xr.open_dataset("/g/data/eg3/ab4502/ExtremeWind/trends/spatial_hist_trend_"+season+"_dcp.nc")
			...
			if season=="DJF":
				plt.ylabel("CMIP5")
			m.drawcoastlines()
			p=m.contourf(x, y, np.where(lsm==1, f1["cmip_trend"]*3, np.nan), levels = np.linspace(vmin,vmax,11),\
                                colors=plt.get_cmap("RdBu_r")(np.linspace(0,1,11)), extend="both")
			...
			plt.subplot(6,4,pos[cnt-1])
			f2 = xr.open_dataset("/g/data/eg3/ab4502/ExtremeWind/trends/spatial_hist_trend_"+season+"_cs6.nc")
			if season=="DJF":
				plt.ylabel("ERA5")
			m.drawcoastlines()
			...
			if season=="DJF":
				plt.ylabel("CMIP5")
			m.drawcoastlines()
			p=m.contourf(x, y, np.where(lsm==1, f2["cmip_trend"]*3, np.nan), levels = np.linspace(vmin,vmax,11),\
                                colors=plt.get_cmap("RdBu_r")(np.linspace(0,1,11)), extend="both")
			...
			plt.subplot(6,4,pos[cnt-1])
			f3 = xr.open_dataset("/g/data/eg3/ab4502/ExtremeWind/trends/spatial_hist_trend_"+season+"_mu_cape.nc")
			if season=="DJF":
				plt.ylabel("ERA5")
			m.drawcoastlines()
			...
			if season=="DJF":
				plt.ylabel("CMIP5")
			m.drawcoastlines()
			p=m.contourf(x, y, np.where(lsm==1, f3["cmip_trend"]*3, np.nan), levels = np.linspace(vmin,vmax,11),\
                                colors=plt.get_cmap("RdBu_r")(np.linspace(0,1,11)), extend="both")
	...
	plt.savefig("/g/data/eg3/ab4502/figs/CMIP/spatial_trend.png",bbox_inches="tight")
