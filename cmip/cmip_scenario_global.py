...
def get_supercell_pot(mucape,srh01,s06):
	...
	s06 = np.where(s06 > 20, 20, s06)
	s06 = np.where(s06 < 10, 0, s06)
	...
	for i in np.arange(len(data_list)):
		xr.Dataset(data_vars={\
				...
				np.nanmean(data_list[i][da_list[i]["time.month"]==12],\
					axis=0)),
			    "DJF":(("lat","lon"),\
				np.nanmean(data_list[i][np.in1d(da_list[i]["time.month"],[12,1,2])],\
					axis=0)),
			    "MAM":(("lat","lon"),\
				np.nanmean(data_list[i][np.in1d(da_list[i]["time.month"],[3,4,5])],\
					axis=0)),
			    "JJA":(("lat","lon"),\
				np.nanmean(data_list[i][np.in1d(da_list[i]["time.month"],[6,7,8])],\
					axis=0)),
			    "SON":(("lat","lon"),\
				np.nanmean(data_list[i][np.in1d(da_list[i]["time.month"],[9,10,11])],\
					axis=0)),
			    },\
			coords={"lat":lat,"lon":lon}).\
		to_netcdf("/g/data/eg3/ab4502/ExtremeWind/global/regrid_1.5/"+\
...
def calc_logit(model, ensemble, p, lsm, hist_y1, hist_y2, scenario_y1, scenario_y2,\
	...
	if p == "logit_aws":
		...
		vars = ["dcape","mu_cape","s06","Umean06"]
	elif p == "scp":
		vars = ["mu_cape","srh01_left","s06"]
	elif p == "cs6":
		vars = ["mu_cape", "s06"]
	elif p == "t_totals":
		vars = ["ta850", "ta500", "dp850"]
	elif p == "eff_sherb":
		vars = ["ebwd", "lr03", "lr700_500"]
	...
	for var_list in [var_list_hist, var_list_scenario]:
		if p == "logit_aws":
			...
			z = (var_list[0] / 980.) * (var_list[1] / 2000.) * ((var_list[2]*1.944) / 20.) * ((var_list[3]*1.944) / 16.)
		elif p == "scp":
			z = get_supercell_pot(var_list[0], var_list[1], var_list[2])
		elif p == "cs6":
			z = var_list[0] * np.power(var_list[1], 1.67)
		elif p == "t_totals":
			v_totals = var_list[0] - var_list[1]
			c_totals = var_list[2] - var_list[1]
			z = v_totals + c_totals
		elif p == "eff_sherb":
			z = (var_list[0] / 27.) * (var_list[1] / 5.2) * (var_list[2] / 5.6)
		if p in ["dcp", "scp", "cs6","t_totals","eff_sherb"]:
			out_logit.append(z)
		else:
...
def load_all_qm_combined(data_hist, data_scenario, models, p, lsm, replace_zeros,\
        ...
        for i in np.arange(len(models)):    
				...
				hist_y1, hist_y2, experiment="historical")
                        if p == "mu_cape":
                                dmax_hist = daily_max_qm([data_hist[i]], [data_hist[i]], [models[i]], p, None)[0][p]
                                save_seasonal_freq([dmax_hist.values], \
                                    [dmax_hist], 230, [models[i]], p,\
                                    data_hist[0].lon.values, data_hist[0].lat.values,\
                                    hist_y1, hist_y2, experiment="historical")
				...
				scenario_y1, scenario_y2, experiment=experiment)
                        if p == "mu_cape":
                                dmax_hist = daily_max_qm([model_xhat_hist], [data_hist[i]], [models[i]], p, None)[0][p]
                                dmax_scenario = daily_max_qm([model_xhat_scenario], [data_scenario[i]], [models[i]], p, None)[0][p]
                                save_seasonal_freq([dmax_hist.values], \
                                    [dmax_hist], 230, [models[i]], p,\
                                    data_hist[0].lon.values, data_hist[0].lat.values,\
                                    hist_y1, hist_y2, experiment="historical")
                                save_seasonal_freq([dmax_scenario.values], \
                                    [dmax_scenario], 230, [models[i]], p,\
                                    data_hist[0].lon.values, data_hist[0].lat.values,\
                                    scenario_y1, scenario_y2, experiment=experiment)
...
if __name__ == "__main__":
	...
	if p in ["srhe_left","ml_cape","mu_cape","dcape","Uwindinf","eff_lcl","ebwd"]:
	        replace_zeros=True
	else:
                replace_zeros=False
	lsm=args.lsm
	...
	if not mean_only:
		if p in ["logit_sta","logit_aws","logit_aws_barra","dcp","scp","cs6","t_totals","eff_sherb"]:

			#This code block loads quantile-mapped ingredients, and computes diagnositcs/logit. equations
		...
		try:
			...
			print("Using frequency for plot...")
		except:
