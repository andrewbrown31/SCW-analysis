...
@check_cape_args()
@cape_left_iter()
@cast_type(arg_idxs=(0, 1, 2, 3, 4, 5), outviews=("capeview", "cinview","lfcview","lclview"\
	,"elview"))
@extract_and_transpose(outviews=("capeview", "cinview","lfcview","lclview","elview"))
def _cape(p_hpa, tk, qv, ht, ter, sfp, missing, i3dflag, ter_follow,
          psafile=psafilepath(), capeview=None,  cinview=None, lfcview=None, lclview=None,\
		elview=None):
    ...
    if lfcview is None:
        lfcview = np.zeros(p_hpa.shape[0:3], p_hpa.dtype, order="F")

    if lclview is None:
        lclview = np.zeros(p_hpa.shape[0:3], p_hpa.dtype, order="F")

    if elview is None:
        elview = np.zeros(p_hpa.shape[0:3], p_hpa.dtype, order="F")
